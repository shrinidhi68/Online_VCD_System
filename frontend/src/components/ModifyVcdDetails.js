...
function App() {
  ...
  useEffect(() => {
    AdminService.getByIdVcd(id).then((response) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    AdminService.modifyVcd(vcdDetails)
      .then((response) => {
        alert(response.data);
        setredirect(true);
      })
  ...
  if (redirect) {
    return <Navigate replace to="/vcdDetails" />;
  } else {
    return (
      <>
        <AdminHeader />
        <center>
          <div>
            <h2 style={{ backgroundColor: "green" }}>Modify Vcd</h2>
          </div>
        </center>
        <br></br>
        <Form onSubmit={handleSubmit}>
          <Row className="justify-content-center">
            <Form.Group as={Col} md="3" controlId="validationCustom01">
              <Form.Label> Vcd Id</Form.Label>
              <Form.Control
                required
                type="number"
                placeholder="Vcd Id"
                name="vcdId"
                value={vcdDetails.vcdId}
